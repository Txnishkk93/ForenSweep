    ...
    <section className={cn("py-32", className)}>
      <div className="container mx-auto">
        <footer>
              ...
              <div key={sectionIdx}>
                ...
                <ul className="space-y-4 text-sm text-neutral-500">
                  {section.links.map((link, linkIdx) => (
                    <li
                      key={linkIdx}
                      className="font-medium transition-colors hover:text-neutral-950"
                    >
          ...
          <div className="mt-8 flex flex-col justify-between gap-4 border-t border-neutral-200 pt-8 text-xs font-medium text-neutral-500 md:flex-row md:items-center">
            <p>{copyright}</p>
            <ul className="flex gap-4">
              {legalLinks?.map((link, linkIdx) => (
                <li key={linkIdx} className="underline transition-colors hover:text-neutral-950">
                  <a href={link.href}>{link.name}</a>
                </li>
              ))}
            </ul>
