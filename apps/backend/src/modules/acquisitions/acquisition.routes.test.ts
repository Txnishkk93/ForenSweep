...
test("ZIP signatures and raw image validation reject obvious disguises", async () => {
  const { acquisitionFilename, generatedUploadFilename, hasZipSignature, isRawBinary } = await import("./acquisition.routes.js");
  ...
  assert.equal(acquisitionFilename("New folder.zip"), "New folder.img");
  assert.equal(acquisitionFilename("../../unsafe folder!.zip"), "unsafe folder-.img");
});